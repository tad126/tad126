...
def extract_main_contour(roi_bgr):
    ...
    # Làm mượt biên mask TRƯỚC khi morphology - giảm răng cưa pixel-level
    # (nguyên nhân chính gây solidity/defect dao động giả giữa các lần chụp cùng 1 vật)
    mask = cv2.GaussianBlur(mask, (7, 7), 0)
    _, mask = cv2.threshold(mask, 127, 255, cv2.THRESH_BINARY)

    kernel = np.ones((7, 7), np.uint8)   # tăng kernel (5->7) để mượt hơn nữa
...
def get_shape_features(contour):
    # Làm mượt contour: loại các điểm lởm chởm nhỏ do nhiễu mask, nhưng vết sứt mẻ
    # THẬT (kích thước lớn hơn hẳn epsilon này) vẫn được giữ nguyên, không bị làm mất.
    perimeter_raw = cv2.arcLength(contour, True)
    epsilon = 0.003 * perimeter_raw
...
def classify_by_shape(features, stats):
    ...
    Dùng cơ chế BỎ PHIẾU CÓ TRỌNG SỐ thay vì chỉ cần 1 chỉ số lệch là kết luận NG ngay -
    giúp giảm báo động giả do 1 chỉ số bị nhiễu nhẹ (ánh sáng, góc chụp...).
    Trả về (result, reasons).
    """
    if stats is None:
        return "CHƯA ĐỦ MẪU", []

    reasons = []
    votes = 0  # cần >= 2 phiếu mới kết luận NG

    # --- Solidity và perimeter_ratio: mỗi cái lệch mạnh (z > threshold) được 1 phiếu ---
    ...
    # --- Defect depth: dấu hiệu TRỰC TIẾP nhất của sứt mẻ -> tính 2 phiếu (đủ tự quyết NG) ---
...
class MachineVisionApp:
    ...
    def test_sample_debug(self):
        """
        Test 1 sản phẩm bất kỳ (đã biết trước là OK hay NG) mà KHÔNG lưu vào bộ mẫu.
        In chi tiết z-score từng chỉ số ra console - dùng để tự tay tìm threshold phù hợp.
        """
        if self.current_frame is None:
            return
        roi = self._get_first_roi(self.current_frame.copy())
        ...
        print(f"\n===== [TEST DEBUG] =====")
        for key in ["solidity", "perimeter_ratio", "max_defect_depth"]:
