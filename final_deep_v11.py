...
FEATURE_MIN_STD = {
    "solidity": 0.02,      # HA tu 0.008: std_thuc do duoc chi 0.0037 - san cu dang gap ~2.2 lan
    "perimeter_ratio": 0.02,  # HA tu 0.012: std_thuc do duoc chi 0.0035 - san cu dang gap ~3.4 lan
}
...
RADIAL_MIN_CONSEC_BINS = 7      # HA tu 5: du lieu thuc te cho thay mau NG that chi tao ra
...
class MachineVisionApp:
    def __init__(self, root):
        ...
        # Kết nối ESP32
        self.esp32 = None
        self.esp32_port = '/dev/ttyUSB0'  # Hoặc COM3 trên Windows
        try:
    ...
    def _update_result(self, detections, cam_id):
        ...
        if hasattr(self, 'send_esp32_command'):
            if batch_ng > 0:
                self.send_esp32_command("NG")
            elif batch_ok > 0:
                self.send_esp32_command("OK")
            # Nếu không có sản phẩm nào trong vùng (tất cả ZONE), không gửi gì cả
    ...
    def check_esp32(self):
        """Lắng nghe tín hiệu TRIG từ ESP32 (khi GPIO34 xuống LOW)."""
            ...
            try:
                if self.esp32.in_waiting > 0:
                    ...
                    if data == "TRIG":
                        ...
                        # Chỉ chụp nếu không đang xử lý và đã qua 1 giây
                        if not self.is_processing and (
                            ...
                            self.root.after(500, self.trigger_cam1)  # Delay 0.5s
            except Exception as e:
