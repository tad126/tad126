...
def compute_stats(ref):
    ...
    if len(samples) < 2:
        return None
    stats = {}
    keys = ["area","solidity", "perimeter_ratio", "defects_count", "max_defect_depth"]
    for k in keys:
...
def classify_by_shape(features, stats):
    ...
    # BỔ SUNG: Kiểm tra diện tích hao hụt (dựa trên Mean Area đã tính trong stats)
    if "area" in stats:
        mean_area = stats["area"]["mean"]
        # Nếu sản phẩm test mất hơn 15% diện tích so với trung bình mẫu OK -> Báo NG luôn
        if features["area"] < mean_area * 0.85:
            reasons.append(f"diện tích hao hụt {((mean_area - features['area'])/mean_area*100):.1f}%")
            return "NG", reasons  # Trả về NG luôn, không cần xét tiếp
