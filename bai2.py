...
SSIM_THRESHOLD = 0.85
AREA_THRESHOLD = 150   # vùng khác biệt (pixel) tối đa cho phép trước khi coi là NG - cần tự test để tìm giá trị chuẩn
CROP_SIZE = (224, 224)
...
def _compute_diff(gray_test, gray_template):
    score, diff_map = ssim(gray_template, gray_test, full=True)
    diff_map_u8 = (diff_map * 255).astype("uint8")
    _, diff_thresh = cv2.threshold(diff_map_u8, 150, 255, cv2.THRESH_BINARY_INV)
    kernel = np.ones((3, 3), np.uint8)
    diff_thresh = cv2.morphologyEx(diff_thresh, cv2.MORPH_OPEN, kernel)
    contours, _ = cv2.findContours(diff_thresh, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
    max_diff_area = max([cv2.contourArea(c) for c in contours], default=0)
    return score, diff_map_u8, max_diff_area


def compare_with_template(straight_crop, template_img, ssim_threshold=SSIM_THRESHOLD, area_threshold=AREA_THRESHOLD):
    """
    Vì bước align (xoay thẳng) không đảm bảo luôn ra ĐÚNG cùng hướng với template
    (có thể lệch 90/180/270 độ tuỳ hướng vật thể ban đầu), ta thử cả 4 hướng xoay
    và chọn hướng khớp nhất (diff_area nhỏ nhất) làm kết quả so sánh cuối cùng.
    """
    ...
    best_score = -1
    best_diff_map = None
    best_diff_area = 999999
    rotations = [
        straight_crop,
        cv2.rotate(straight_crop, cv2.ROTATE_90_CLOCKWISE),
        cv2.rotate(straight_crop, cv2.ROTATE_180),
        cv2.rotate(straight_crop, cv2.ROTATE_90_COUNTERCLOCKWISE),
    ]

    for rotated in rotations:
        resized_test = letterbox_resize(rotated, CROP_SIZE)
        gray_test = cv2.cvtColor(resized_test, cv2.COLOR_BGR2GRAY)
        score, diff_map_u8, diff_area = _compute_diff(gray_test, gray_template)

        # Chọn hướng có diff_area NHỎ NHẤT (khớp nhất với template)
        if diff_area < best_diff_area:
            best_diff_area = diff_area
            best_score = score
            best_diff_map = diff_map_u8

    result = "NG" if (best_score < ssim_threshold or best_diff_area > area_threshold) else "OK"
    return result, best_score, best_diff_map, best_diff_area
...
class MachineVisionApp:
    def __init__(self, root):
        ...
        self.model = YOLO("C:/Downloads/check sp_v2-20260712T152639Z-2-001/check sp_v2/weights/best_ncnn_model")
    ...
    def setup_ui(self):
        ...
        self._param_row(control_panel, "Area Threshold", str(AREA_THRESHOLD))
    ...
    def run_inspection(self):
        ...
        for idx, box in enumerate(boxes):
            ...
            result, score, diff_map, diff_area = compare_with_template(straight_crop, self.template_img)
            ...
            # In ra console để tiện xem số liệu và tự tinh chỉnh SSIM_THRESHOLD / area_threshold
            print(f"Sản phẩm #{idx + 1}: SSIM={score:.3f} | diff_area={diff_area:.0f} => {result}")

            cv2.rectangle(display_frame, (x1, y1), (x2, y2), color, 2)
            cv2.putText(display_frame, f"{idx + 1}", (x1, max(15, y1 - 10)),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.7, color, 2)
            cv2.putText(display_frame, f"{result} S:{score:.2f} A:{diff_area:.0f}",
