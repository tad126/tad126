...
def extract_aligned_contour(roi_bgr):
    ...
    if USE_HSV_MASK:
        ...
        # === GIÃN NỞ MẶT NẠ (Dilate) để bù lại cạnh bị phản quang ===
    ...
    # --- XOAY ẢNH (ALIGN) ĐÃ SỬA LỖI CẮT ẢNH ---
    ...
    # 1. Tính kích thước khung mới (Tránh bị cắt ảnh)
    abs_cos = abs(np.cos(np.deg2rad(angle)))
    abs_sin = abs(np.sin(np.deg2rad(angle)))
    new_w = int(h * abs_sin + w * abs_cos)
    new_h = int(h * abs_cos + w * abs_sin)

    # 2. Tạo ma trận xoay và dịch chuyển ảnh về giữa khung mới
    ...
    # Fallback nếu xoay bị méo
    if cv2.contourArea(final_contour) < cv2.contourArea(main_contour) * 0.8:
        return main_contour
...
def classify_by_shape(features, stats):
    ...
    if "norm_area" in stats:
        ...
        if z > Z_SCORE_THRESHOLD and features["norm_area"] < mean_norm * 0.97:
    ...
    def run_inspection(self):
        ...
        for idx, box in enumerate(boxes):
            ...
            if result == "NG":
                ...
                cv2.imwrite(f"{ng_folder}/NG_{timestamp}_{idx}.jpg", roi)
            elif result == "OK":
                self.ok_count += 1
                batch_ok += 1
                color = (0, 255, 0)
