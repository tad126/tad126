...
# 1. QUAN TRỌNG: Thu hẹp vùng kiểm tra (20% - 80%) để chặn lỗi ở rìa màn hình
ZONE_X_MIN, ZONE_X_MAX = 0.20, 0.80
ZONE_Y_MIN, ZONE_Y_MAX = 0.20, 0.80

# Ngưỡng màu cho sản phẩm xanh
LOWER_COLOR = np.array([82, 127, 92])
UPPER_COLOR = np.array([179, 255, 255])

# 2. CỜ ĐIỀU KHIỂN: True khi test trên nền gỗ, False khi chạy băng tải xanh
USE_HSV_MASK = True

# ============================================================
# BƯỚC 0: Đo góc đỉnh nhọn (Bắt lỗi bo tròn)
# ============================================================
ANGLE_SHARP_THRESHOLD = 145  # Điều chỉnh sau khi test


def check_corner_angle(contour):
    if contour is None or len(contour) < 5:
        return 0.0, None
    min_y_idx = np.argmin(contour[:, :, 1])
    tip_point = contour[min_y_idx][0]
    total_points = len(contour)
    step = 8
    p1 = contour[(min_y_idx - step) % total_points][0]
    p2 = contour[(min_y_idx + step) % total_points][0]
    v1 = p1 - tip_point
    v2 = p2 - tip_point
    cos_theta = np.dot(v1, v2) / (np.linalg.norm(v1) * np.linalg.norm(v2) + 1e-6)
    angle_deg = np.degrees(np.arccos(np.clip(cos_theta, -1.0, 1.0)))
    return angle_deg, tip_point
...
def extract_aligned_contour(roi_bgr):
    ...
    if USE_HSV_MASK:
        ...
        mask_hsv = cv2.dilate(mask_hsv, dilate_kernel, iterations=2)
    else:
        h, w = roi_bgr.shape[:2]
        mask_hsv = np.ones((h, w), dtype=np.uint8) * 255

    # --- TÌM CONTOUR TRỰC TIẾP TRÊN MASK ---
    contours, _ = cv2.findContours(mask_hsv, cv2.RETR_CCOMP, cv2.CHAIN_APPROX_SIMPLE)
    ...
    # Tính kích thước khung mới tránh cắt ảnh
    ...
    rotated_roi = cv2.warpAffine(roi_bgr, M, (new_w, new_h), flags=cv2.INTER_LINEAR)

    # --- TÌM LẠI CONTOUR SAU XOAY LẦN 1 ---
    ...
    if USE_HSV_MASK:
        ...
        h_rot, w_rot = rotated_roi.shape[:2]
        mask_hsv_rot = np.ones((h_rot, w_rot), dtype=np.uint8) * 255
    ...
    # ==========================================================
    # BỔ SUNG: Kiểm tra và sửa lỗi lộn ngược (Đầu nhọn hướng xuống)
    # ==========================================================
    if final_contour is not None and len(final_contour) > 0:
        # Tìm điểm có tọa độ y nhỏ nhất (đỉnh cao nhất)
        min_y_idx = np.argmin(final_contour[:, :, 1])
        tip_y = final_contour[min_y_idx][0][1]

        # Nếu đỉnh nhọn đang nằm ở nửa dưới khung hình (tức bị lộn ngược)
        if tip_y > new_h / 2:
            # Xoay ảnh thêm 180 độ
            M_180 = cv2.getRotationMatrix2D((new_w / 2, new_h / 2), 180, 1.0)
            rotated_roi = cv2.warpAffine(rotated_roi, M_180, (new_w, new_h), flags=cv2.INTER_LINEAR)

            # Tìm lại contour lần cuối trên ảnh đã xoay 180 độ
            gray_rot = cv2.cvtColor(rotated_roi, cv2.COLOR_BGR2GRAY)
            if USE_HSV_MASK:
                hsv_rot = cv2.cvtColor(rotated_roi, cv2.COLOR_BGR2HSV)
                mask_hsv_rot = cv2.inRange(hsv_rot, LOWER_COLOR, UPPER_COLOR)
...
def compute_stats(ref):
    ...
    if len(samples) < 2:
        return None
    stats = {}
    keys = ["norm_area", "centroid_shift", "solidity", "perimeter_ratio", "defects_count", "max_defect_depth",
            "mean_defect_depth"]
    for k in keys:
...
def classify_by_shape(features, stats):
    ...
    # Dùng Z-score cho norm_area kết hợp ngưỡng cứng an toàn
    if "norm_area" in stats:
        mean_norm = stats["norm_area"]["mean"]
        std_norm = stats["norm_area"]["std"]
        effective_std_norm = max(std_norm, 0.005)
        z = abs(features["norm_area"] - mean_norm) / effective_std_norm
        # Đã bỏ điều kiện "and features["norm_area"] < mean_norm * 0.97"
        if z > Z_SCORE_THRESHOLD:
            reasons.append(f"norm_area lệch {z:.1f} lần")
...
class MachineVisionApp:
    def __init__(self, root):
        ...
        # SỬA LỖI ĐƯỜNG DẪN: Chỉ cần đặt file best.pt cùng thư mục với file .py
        ...
        if not os.path.exists(model_path):
            print(f"⚠️ Cảnh báo: Không tìm thấy model tại {model_path}! Hãy đặt file best.pt cùng thư mục.")
            model_path = "best.pt"
    ...
    def open_device(self):
        ...
        # BẬT EXPOSURE CỐ ĐỊNH: Thử giá trị 100 (Có thể chỉnh 50-200 tùy độ sáng)
    ...
    def run_inspection(self):
        ...
        for idx, box in enumerate(boxes):
            ...
            if contour is None:
                ...
                # === BỔ SUNG: Đo góc đỉnh nhọn ===
                corner_angle, _ = check_corner_angle(contour)

                # Nếu góc > ngưỡng -> Báo NG luôn
                if corner_angle > ANGLE_SHARP_THRESHOLD:
                    result = "NG"
                    reasons = [f"đầu nhọn bị bo tròn ({corner_angle:.1f} độ)"]
                else:
                    result, reasons = classify_by_shape(features, self.ref_stats)
            ...
            cv2.putText(display_frame, label_text, (x1, min(frame.shape[0] - 5, y2 + 20)), cv2.FONT_HERSHEY_SIMPLEX,
                        0.5, color, 1)
            ...
            print(
                f"San pham #{idx + 1}: {result} | {feat_note} | sharpness={sharpness:.0f}{blur_note} | ly do: {', '.join(reasons) if reasons else '-'}")
