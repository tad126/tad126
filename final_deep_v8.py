...
# CẤU HÌNH CHUNG
# ============================================================
REFERENCE_PATH = "reference_stats.json"  # Lưu thống kê OK
MIN_OK_SAMPLES = 20
Z_SCORE_THRESHOLD = 5.0
DEFECT_DEPTH_THRESHOLD = 3.0

# --- Sàn (floor) tối thiểu cho STD của từng đặc trưng ---
FEATURE_MIN_STD = {
    "solidity": 0.008,
...
# Vùng kiểm tra an toàn (20% - 80% màn hình)
ZONE_X_MIN, ZONE_X_MAX = 0.20, 0.80
ZONE_Y_MIN, ZONE_Y_MAX = 0.20, 0.80

# ============================================================
# CẤU HÌNH HSV RIÊNG CHO 2 CAMERA
# ============================================================
HSV_CONFIG = {
    "cam1": {
...
# ============================================================
# BƯỚC 1: XỬ LÝ ẢNH (LẤY CONTOUR) – nhận thêm tham số HSV
# ============================================================
...
def extract_main_contour(roi_bgr, lower_color, upper_color):
    ...
    if hierarchy is not None:
        ...
        for i, h in enumerate(hierarchy[0]):
            if h[3] != -1:  # Là contour con (lỗ bên trong)
...
# ============================================================
# BƯỚC 2: TRÍCH XUẤT ĐẶC TRƯNG HÌNH HỌC (Z-Score)
# ============================================================
...
# ============================================================
# BƯỚC 3: Z-SCORE VÀ BỘ LỌC LỖI NHỎ
# ============================================================
def load_reference():
...
# ============================================================
# BƯỚC 4: GIAO DIỆN VÀ LOGIC CHÍNH (2 CAMERA + 2 HSV + CHỌN CAM GIAO DIỆN)
# ============================================================
BG_DARK = "#3c3f41"
BG_PANEL = "#e8e8e8"
BG_IMAGE = "#4a4a4a"
...
class MachineVisionApp:
    def __init__(self, root):
        self.root = root
        self.root.title("Machine Vision Inspection System - Dual Camera")
        ...
        if not os.path.exists(model_path):
            print(f"Cảnh báo: Không tìm thấy model tại {model_path}! Hãy đặt file best.pt cùng thư mục.")
            model_path = "best.pt"
    ...
    def _section_label(self, parent, text):
        tk.Label(parent, text=text, font=tkfont.Font(size=9, weight="bold"), bg=BG_PANEL, fg="#333").pack(anchor="w",
                                                                                                          padx=8,
                                                                                                          pady=(10, 2))
    ...
    def open_device(self):
        ...
        self.close_device()  # Đóng cũ trước khi mở mới
    ...
    def _get_first_roi(self, frame, lower_color, upper_color):
        ...
        for box in boxes:
            ...
            contour, _, _ = extract_main_contour(roi, lower_color, upper_color)
    ...
    def _inspect_frame(self, frame, lower_color, upper_color):
        ...
        boxes = results[0].boxes
        if len(boxes) == 0:
            return "NG", ["không phát hiện sản phẩm"], None
        box = boxes[0]
        x1, y1, x2, y2 = map(int, box.xyxy[0])
        pad = 5
        x1p, y1p = max(0, x1 - pad), max(0, y1 - pad)
        ...
        if contour is None:
            return "NG", ["không tách được contour"], None
        ...
        return result, reasons, features

    # ==========================================================
    # LOGIC TRIGGER
    # ==========================================================
    def _update_result(self, result, reasons, features, cam_id):
        if result == "NG":
            self.ng_count += 1
        else:
            self.ok_count += 1
        ...
        color = (0, 0, 255) if result == "NG" else (0, 255, 0)
        results = self.model(frame, imgsz=320, conf=0.5, iou=0.45, verbose=False)
        boxes = results[0].boxes
        if len(boxes) > 0:
            x1, y1, x2, y2 = map(int, boxes[0].xyxy[0])
            cv2.rectangle(display_frame, (x1, y1), (x2, y2), color, 2)
            cv2.putText(display_frame, result, (x1, y1 - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.7, color, 2)

        self.display_image(self.result_label_img, display_frame)
        self.count_label.config(text=f"Cam{cam_id} - Products Detected: {len(boxes)} | Result: {result}")
        print(f"Cam {cam_id}: {result} | reasons: {', '.join(reasons) if reasons else '-'}")

    def trigger_cam1(self):
        if self.current_frame1 is None: return
        result, reasons, features = self._inspect_frame(self.current_frame1, HSV_CONFIG["cam1"]["lower"],
                                                        HSV_CONFIG["cam1"]["upper"])
        self._update_result(result, reasons, features, 1)

    def trigger_cam2(self):
        if self.current_frame2 is None: return
        result, reasons, features = self._inspect_frame(self.current_frame2, HSV_CONFIG["cam2"]["lower"],
                                                        HSV_CONFIG["cam2"]["upper"])
        self._update_result(result, reasons, features, 2)
    ...
    def clear_samples(self):
        ...
        self.refresh_sample_count()
        print("Đã xóa toàn bộ mẫu tham chiếu.")
