...
def extract_main_contour(roi_bgr):
    ...
    # QUAN TRONG: Sua RETR_EXTERNAL thanh RETR_CCOMP de giu lai cac lo ben trong
...
def get_shape_features(contour):
    # === CHÈN ĐOẠN NÀY VÀO ĐẦU HÀM ===
    # Làm mịn contour để loại bỏ răng cưa trước khi tính đặc trưng
    ...
    # Lam min contour de giam răng cưa gay ng gia
    epsilon = 0.002 * cv2.arcLength(contour, True)
...
# ============================================================
# BUOC 3: Z-SCORE VA BO LOC LỖI NHỎ (AREA RATIO)
# ============================================================
def load_reference():
...
def classify_by_shape(features, stats):
    ...
    # 1. Z-score cho solidity va perimeter_ratio
    ...
    # 3. Bổ sung: So sánh diện tích tuyệt đối (Area) để bắt lỗi nhỏ
    # Đặt ngưỡng giảm 3% diện tích so với trung bình là NG
    if "area" in stats:
...
class MachineVisionApp:
    def __init__(self, root):
        ...
        # SUA LOI: Duong dan model tuyet doi -> chuyen ve relative de chay may khac
    ...
    def run_inspection(self):
        ...
        # BO SUNG: Bat NG ngay neu YOLO khong detect ra gi
        if len(boxes) == 0:
            self.ng_count += 1
