...
RADIAL_BINS = 72               # Do phan giai goc: 360/120 = 3 do/lat (tang tu 72 vi camera
                                 # dat gan hon -> vat co nhieu pixel hon, du chi tiet cho do
                                 # phan giai cao hon ma khong bi thieu diem ho tro moi lat)
RADIAL_MIN_STD = 0.012          # HA tu 0.012 (cung ly do voi FEATURE_MIN_STD) - hay chay lai
                                 # "Test mau" de xem std_thuc thuc te tung lat va tinh chinh tiep
RADIAL_Z_THRESHOLD = 3.0        # Lat bi coi la "hut" neu ban kinh ngan hon chuan > nguong nay lan std
RADIAL_MIN_CONSEC_BINS = 3        # HA tu 5: du lieu thuc te cho thay mau NG that chi tao ra
                                 # chuoi 3 lat lien tiep vuot nguong (9 do), 5 la qua khat
RADIAL_MAX_PHASE_SHIFT = 20    # KHONG CON DUNG: align_profile_phase gio mac dinh tim tren
...
def align_profile_phase(test_profile, ref_profile, max_shift=RADIAL_MAX_PHASE_SHIFT):
    ...
    max_shift mac dinh = 10 (bin) tuong duong ~30 do (voi 120 bin). Gioi han nay ngan
    thuat toan chon nhung pha "ao" qua xa, gay sai lech cho mau OK."""
    n = len(ref_profile)
    if max_shift is None:
        candidates = range(n)
    else:
        # Gioi han tim kiem trong khoang [-max_shift, max_shift]
        candidates = list(range(-max_shift, max_shift + 1))
    ...
    # Chuan hoa shift ve khoang [-n/2, n/2] cho de doc
    if best_shift > n // 2:
        best_shift -= n
...
class MachineVisionApp:
    def __init__(self, root):
        ...
        # Kết nối ESP32
        self.esp32 = None
        self.esp32_port = 'COM5'  # Hoặc COM3 trên Windows
        try:
            self.esp32 = serial.Serial(self.esp32_port, 115200, timeout=0.01)
            print(f"Đã kết nối ESP32 tại {self.esp32_port}")
        ...
        # Biến chống trigger lặp
        self.is_processing = False
        self.last_trigger_time = 0
        self.min_trigger_interval = 1.0
        # Thêm 2 biến bật/tắt camera
        self.enable_cam1 = tk.BooleanVar(value=True)
        self.enable_cam2 = tk.BooleanVar(value=False)

        self.setup_ui()
        self.search_device()  # Tự động quét camera khi khởi động

        # === TỰ ĐỘNG MỞ CAMERA 1 (VÀ CAMERA 2 NẾU ĐƯỢC BẬT) ===
        if self.available_devices:
            # Đảm bảo chọn Camera 1 trong dropdown (thường đã được set ở search_device)
            self.device_combo1.current(0)
            if len(self.available_devices) > 1:
                self.device_combo2.current(1)
            else:
                self.device_combo2.current(0)
            # Gọi hàm mở camera tự động
            self.open_device()
            print("Đã tự động mở camera.")
        else:
            print("Không tìm thấy camera nào để tự động mở!")

        # === BẮT ĐẦU CÁC VÒNG LẶP NỀN ===
        self.update_camera()
        self.refresh_sample_count()

        # Nếu có kết nối Arduino/ESP32, tự động bắt đầu lắng nghe cảm biến
        if hasattr(self, 'check_esp32'):
            self.check_esp32()
        elif hasattr(self, 'serial_loop'):
            # Nếu bạn dùng luồng riêng (threading) thì đã được start trong __init__ rồi
            pass
    ...
    def _get_first_roi(self, frame, lower_color, upper_color):
        ...
        # CHI LAY 1 SAN PHAM DUY NHAT: neu co nhieu box trong vung kiem tra
        # (VD 2 san pham lot vao khung cung luc), chon box co confidence cao nhat,
        # bo qua cac box con lai de tranh lay nham/lay lan san pham thu 2.
        best_box = None
        best_conf = -1.0
        for box in boxes:
    ...
    def _inspect_frame(self, frame, lower_color, upper_color):
        """CHI kiem tra 1 san pham DUY NHAT moi lan trigger (box co confidence cao nhat
        trong vung kiem tra), du YOLO co phat hien nhieu vat trong khung cung luc,
        de tranh bat nham/lan san pham thu 2."""
        results = self.model(frame, imgsz=320, conf=0.5, iou=0.45, verbose=False)
        boxes = results[0].boxes
        detections = []
        if len(boxes) == 0:
        ...
        # Tach box hop le (trong vung kiem tra) va box ngoai vung (ZONE)
        valid_boxes = []  # (conf, x1, y1, x2, y2)
        zone_boxes = []  # (x1, y1, x2, y2)
        for box in boxes:
            x1, y1, x2, y2 = map(int, box.xyxy[0])
            if not is_in_inspection_zone(x1, y1, x2, y2, frame.shape[1], frame.shape[0]):
                zone_boxes.append((x1, y1, x2, y2))
                continue
            valid_boxes.append((float(box.conf[0]), x1, y1, x2, y2))

        if not valid_boxes:
            # Khong co box nao trong vung kiem tra -> chi bao ZONE de UI biet,
            # khong tinh OK/NG (cho trigger lai khi vat vao dung vi tri).
            for (x1, y1, x2, y2) in zone_boxes:
                detections.append({"bbox": (x1, y1, x2, y2), "result": "ZONE", "reasons": []})
            return detections

        # CHI CHON 1 BOX DUY NHAT: box co confidence cao nhat trong vung kiem tra,
        # bo qua moi box con lai (kha nang la san pham thu 2 lot vao khung).
    ...
    def _update_result(self, detections, cam_id):
        """
        Cập nhật kết quả kiểm tra lên giao diện và gửi tín hiệu cho ESP32.

        Args:
            detections (list): Danh sách các dict chứa 'bbox', 'result', 'reasons'.
            cam_id (int): 1 cho Camera 1, 2 cho Camera 2.
        """
        # Chọn khung hình tương ứng
        frame = self.current_frame1 if cam_id == 1 else self.current_frame2
        display_frame = frame.copy()

        batch_ok, batch_ng = 0, 0

        # Duyệt qua từng sản phẩm phát hiện được
        for idx, det in enumerate(detections):
            x1, y1, x2, y2 = det["bbox"]
            result = det["result"]

            # Xử lý sản phẩm nằm ngoài vùng kiểm tra
            if result == "ZONE":
            ...
            # Vẽ khung và nhãn
            cv2.rectangle(display_frame, (x1, y1), (x2, y2), color, 2)
            cv2.putText(display_frame, f"{idx + 1}",
                        (x1, max(15, y1 - 10)),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.7, color, 2)
            cv2.putText(display_frame, result,
                        (x1, min(frame.shape[0] - 5, y2 + 20)),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.5, color, 1)

            # In log ra console
            reasons_text = ', '.join(det['reasons']) if det['reasons'] else '-'
            print(f"Cam{cam_id} #{idx + 1}: {result} | ly do: {reasons_text}")
        ...
        # Tính toán kết quả tổng hợp cho lô sản phẩm này
        batch_ng = sum(1 for det in detections if det["result"] == "NG")
        batch_ok = sum(1 for det in detections if det["result"] == "OK")
        if batch_ng > 0:
            # Lên lịch gửi "NG" sau 1 giây
            self.root.after(1000, self.send_esp32_command, "NG")
        elif batch_ok > 0:
            # Lên lịch gửi "OK" sau 1 giây
            self.root.after(1000, self.send_esp32_command, "OK")
    def trigger_cam1(self):
        ...
        if self.current_frame1 is None:
            self.is_processing = False
            return
        detections = self._inspect_frame(self.current_frame1, HSV_CONFIG["cam1"]["lower"], HSV_CONFIG["cam1"]["upper"])
        if not detections:
            h, w = self.current_frame1.shape[:2]
            detections = [{"bbox": (0, 0, w, h), "result": "NG",
                        "reasons": ["khong phat hien san pham (model tra ve rong)"]}]
    ...
    def trigger_cam2(self):
        ...
        if self.current_frame2 is None:
            self.is_processing = False
            return
        detections = self._inspect_frame(self.current_frame2, HSV_CONFIG["cam2"]["lower"], HSV_CONFIG["cam2"]["upper"])
        self._update_result(detections, 2)
        self.is_processing = False

    def check_esp32(self):
        if self.esp32 and self.esp32.is_open:
            try:
                if self.esp32.in_waiting > 0:
                    data = self.esp32.readline().decode().strip()
                    if data == "TRIG":
                        ...
                        if not self.is_processing and (
                            ...
                            print("Nhận TRIG, chờ 0.5s rồi chụp ảnh...")
                            self.root.after(500, self.trigger_cam1)
            except (serial.SerialException, OSError) as e:
                print("Mất kết nối Serial, thử kết nối lại:", e)
                try:
                    self.esp32.close()
                except Exception:
                    pass
                try:
                    self.esp32 = serial.Serial(self.esp32_port, 115200, timeout=0.01)
                    print(f"Đã kết nối lại ESP32 tại {self.esp32_port}")
                except Exception as e2:
                    print("Chưa kết nối lại được:", e2)
        else:
            # esp32 đang None hoặc đã đóng -> thử mở lại định kỳ
            try:
                self.esp32 = serial.Serial(self.esp32_port, 115200, timeout=0.01)
                print(f"Đã kết nối lại ESP32 tại {self.esp32_port}")
            except Exception:
                pass
        self.root.after(20, self.check_esp32)

    def send_esp32_command(self, cmd):
        """Gửi lệnh OK/NG cho ESP32."""
        if self.esp32 and self.esp32.is_open:
            try:
                self.esp32.write((cmd + '\n').encode('utf-8'))
                print(f"Đã gửi lệnh '{cmd}' cho ESP32")
            except Exception as e:
